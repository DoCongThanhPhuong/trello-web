...
function ActiveCard() {
  ...
  const onUpdateCardTitle = (newTitle) => {
    updateCardData({ title: newTitle.trim() })
  }
  ...
  const onUploadCardCover = (event) => {
    ...
    toast.promise(
      updateCardData(reqData).finally(() => (error.target.value = '')),
      { pending: 'Updating...' }
    )
  }
  ...
  const onUpdateCardMembers = (incomingUserInfo) => {
    updateCardData({ incomingUserInfo })
  }
              ...
              {!activeCard?.memberIds?.includes(user._id) && (
                <SidebarItem
                  ...
                  onClick={() =>
                    onUpdateCardMembers({
                      userId: user._id,
                      action: CARD_MEMBERS_ACTION.ADD
                    })
                  }
                >
